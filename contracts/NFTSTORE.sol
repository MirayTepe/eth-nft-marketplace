...
contract NFTSTORE is ERC721URIStorage {
    ...
    function executeSale(uint256 tokenId) public payable {
        ...
        listing.seller.transfer(msg.value - listingFee);
    }
    ...
    function getAllListedNFTs() public view returns (NFTListing[] memory) {
        ...
        for (uint256 i = 1; i <= totalNFTCount; i++) {
            listedNFTs[i - 1] = tokenIdToListing[i];
        }
    ...
    function getMyNFTs() public view returns(NFTListing[] memory) {
        ...
        // İlk döngü, sahip olduğunuz veya satıcısı olduğunuz NFT sayısını bulur
        for (uint256 i = 1; i <= totalNFTCount; i++) {
            if (tokenIdToListing[i].owner == msg.sender || tokenIdToListing[i].seller == msg.sender) {
                myNFTCount++;
            }
        }
        ...
        // İkinci döngü, sahip olduğunuz veya satıcısı olduğunuz NFT'leri toplar
        for (uint256 i = 1; i <= totalNFTCount; i++) {
            if (tokenIdToListing[i].owner == msg.sender || tokenIdToListing[i].seller == msg.sender) {
                myNFTs[currentIndex] = tokenIdToListing[i];
                currentIndex++;
            }
        }
